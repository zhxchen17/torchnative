...
impl SuppressTokens {
    ...
    fn apply(&self, mut logits: PyTensor, tokens: PyTensor) {
...
struct PyTorchInference {
    ...
    kv_cache: HashMap<String, PyTensor>,
}
...
impl PyTorchInference {
    ...
    fn new(initial_token_length: i64) -> Self {
        PyTorchInference {
            ...
            kv_cache: HashMap::default(),
        }
    }

    // fn logits(&self, mut tokens: PyTensor, audio_features: PyTensor) -> PyTensor {
...
type GetCallSpecFn = unsafe extern "C" fn(ContainerHandle, *mut *const libc::c_char, *mut *const libc::c_char) -> AOTIRuntimeError;
...
impl AOTIModel {
    ...
    fn call_spec(&self) -> anyhow::Result<(String, String)> {
        unsafe {
            ...
            // TODO leaky
            Ok((
                unsafe {std::ffi::CStr::from_ptr(in_spec)}.to_str()?.to_owned(),
                unsafe {std::ffi::CStr::from_ptr(out_spec)}.to_str()?.to_owned(),
            ))
        }
    }
    ...
    fn __call__(
        &self,
        mut py_args: &Bound<'_, PyTuple>,
    ) -> anyhow::Result<Vec<PyTensor>> {
        // TODO tree flatten
        let mut inputs = vec![];
        for a in py_args {
